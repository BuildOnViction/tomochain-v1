...
func (s *Snapshot) store(db OrderDao) error {
	...
	return db.Put(append([]byte(snapshotPrefix), s.Hash[:]...), &blob, NonDryrunMode)
}
...
func newSnapshot(tomox *TomoX, blockHash common.Hash) (*Snapshot, error) {
	...
	for _, pair := range tomox.listTokenPairs() {
		ob, err = tomox.GetOrderBook(pair, NonDryrunMode)
		if err != nil {
			return nil, err
		}
...
func prepareOrderTreeData(tree *OrderTree) (*OrderTreeSnapshot, error) {
	...
	for _, key := range tree.PriceTree.Keys(NonDryrunMode) {
		priceKeyHash := common.BytesToHash(key)
		bytes, found := tree.PriceTree.Get(key, NonDryrunMode)
		if found {
			var ol *OrderList
			...
			order := ol.GetOrder(ol.Item.HeadOrder, NonDryrunMode)
			for order != nil {
				if byteItem, err = EncodeBytesItem(order.Item); err != nil {
					return nil, err
				}
				items = append(items, byteItem)
				order = order.GetNextOrder(ol, NonDryrunMode)
			}
...
func getSnapshot(db OrderDao, blockHash common.Hash) (*Snapshot, error) {
	...
	blob, err = db.Get(append([]byte(snapshotPrefix), blockHash[:]...), &[]byte{}, NonDryrunMode)
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) RestoreOrderTree(treeSnap *OrderTreeSnapshot, tree *OrderTree) (*OrderTree, error) {
	...
	for _, olSnap := range treeSnap.OrderList {
		...
		if err = tree.SaveOrderList(ol, NonDryrunMode); err != nil {
			return tree, err
		}
		...
		for _, item := range olSnap.OrderItem {
			...
			if err = ol.SaveOrder(order, NonDryrunMode); err != nil {
				return tree, err
			}
		}
