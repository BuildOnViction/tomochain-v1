...
func TestTomoX_Snapshot(t *testing.T) {
	...
	price := CloneBigInt(ether)
	price = price.Mul(price, big.NewInt(99))
	if err = ob.Bids.orderDB.Delete(ob.Bids.getKeyFromPrice(price),NonDryrunMode); err != nil {
		t.Error("Failed to delete order", "price", price)
	}
	// remove order whose OrderId = 4 (ask order)
	price = CloneBigInt(ether)
	price = price.Mul(price, big.NewInt(102))
	if err = ob.Asks.orderDB.Delete(ob.Asks.getKeyFromPrice(price),NonDryrunMode); err != nil {
		t.Error("Failed to delete order", "price", price)
	}
	...
	price = CloneBigInt(ether)
	price = price.Mul(price, big.NewInt(99))
	order := bidTree.GetOrder(GetKeyFromBig(big.NewInt(1)), price, NonDryrunMode)
	if order == nil {
	...
	price = CloneBigInt(ether)
	price = price.Mul(price, big.NewInt(98))
	order = bidTree.GetOrder(GetKeyFromBig(big.NewInt(2)), price, NonDryrunMode)
	if order == nil {
		t.Error("Can not find order", "price", price)
	...
	price = CloneBigInt(ether)
	price = price.Mul(price, big.NewInt(101))
	order = askTree.GetOrder(GetKeyFromBig(big.NewInt(3)), price, NonDryrunMode)
	if order == nil {
		t.Error("Can not find order", "price", price)
	...
	price = CloneBigInt(ether)
	price = price.Mul(price, big.NewInt(102))
	order = askTree.GetOrder(GetKeyFromBig(big.NewInt(4)), price, NonDryrunMode)
	if order == nil {
		t.Error("Can not find order", "price", price)
