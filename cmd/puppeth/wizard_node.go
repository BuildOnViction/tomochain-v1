...
func (w *wizard) deployNode(boot bool) {
	...
	// If the node is a masternode/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based stakers only need an etherbase to stake against
			fmt.Println()
			if infos.etherbase == "" {
				fmt.Printf("What address should the masternode user?\n")
				for {
				...
				fmt.Printf("What address should the masternode user? (default = %s)\n", infos.etherbase)
