...
// sigHash returns the hash which is used as input for the delegated proof-of-stake-voting
...
// DPOS is the delegated delegated proof-of-stake consensus engine proposed to support the
...
// New creates a DPOS delegated proof-of-stake-voting consensus engine with the initial
...
func (c *DPOS) verifyHeaderWithCache(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
...
func (c *DPOS) GetSnapshot(chain consensus.ChainReader, header *types.Header) (*Snapshot, error) {
	number := header.Number.Uint64()
...
func (c *DPOS) StoreSnapshot(snap *Snapshot) error {
...
func (c *DPOS) GetMasternodes(chain consensus.ChainReader, header *types.Header) []common.Address {
...
func (c *DPOS) GetPeriod() uint64 { return c.config.Period }
...
func (c *DPOS) GetValidator(creator common.Address, chain consensus.ChainReader, header *types.Header) (common.Address, error) {
...
func (c *DPOS) UpdateMasternodes(chain consensus.ChainReader, header *types.Header, ms []Masternode) error {
	number := header.Number.Uint64()
	log.Trace("take snapshot", "number", number, "hash", header.Hash())
	// get snapshot
	snap, err := c.snapshot(chain, number, header.Hash(), nil)
...
func (c *DPOS) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	...
	number := header.Number.Uint64()
	rCheckpoint := chain.Config().DPOS.RewardCheckpoint
...
// with.
func (c *DPOS) Authorize(signer common.Address, signFn clique.SignerFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *DPOS) RecoverSigner(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures)
}

func (c *DPOS) RecoverValidator(header *types.Header) (common.Address, error) {
	// If the signature's already cached, return that
	hash := header.Hash()
...
func (c *DPOS) GetMasternodesFromCheckpointHeader(preCheckpointHeader *types.Header, n, e uint64) []common.Address {
