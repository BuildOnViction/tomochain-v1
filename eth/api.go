...
// PublicStakerAPI provides an API to control the masternode.
...
// SubmitWork can be used by external masternode to submit their POW solution. It returns an indication if the work was
// accepted. Note, this is not an indication if the provided work was valid!
func (api *PublicStakerAPI) SubmitWork(nonce types.BlockNonce, solution, digest common.Hash) bool {
	return api.agent.SubmitWork(nonce, digest, solution)
}

// GetWork returns a work package for external masternode. The work package consists of 3 strings
...
// hash rate of all stakers which submit work through this node. It accepts the masternode hash rate and an identifier which
...
// PrivateStakerAPI provides private RPC methods to control the masternode.
// These methods can be abused by external users and must be considered insecure for use by untrusted users.
type PrivateStakerAPI struct {
	e *Ethereum
}

// NewPrivateStakerAPI create a new RPC service which controls the masternode of this node.
func NewPrivateStakerAPI(e *Ethereum) *PrivateStakerAPI {
	return &PrivateStakerAPI{e: e}
}

// Start the masternode with the given number of threads. If threads is nil the number
...
func (api *PrivateStakerAPI) Start(threads *int) error {
	...
	if threads == nil {
		...
		*threads = -1 // Disable the masternode from within
	}
	...
	// Start the masternode and return
	if !api.e.IsStaking() {
...
// SetExtra sets the extra data string that is included when this masternode stakes a block.
...
// SetGasPrice sets the minimum accepted gas price for the masternode.
...
// SetEtherbase sets the etherbase of the masternode
func (api *PrivateStakerAPI) SetEtherbase(etherbase common.Address) bool {
	api.e.SetEtherbase(etherbase)
	return true
}

// GetHashrate returns the current hashrate of the masternode.
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		// the masternode and operate on those
